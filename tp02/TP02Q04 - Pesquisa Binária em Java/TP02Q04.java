...
public class TP02Q04 {
    ...
    public static String verifySearch(String search,String name[], int numSearchs){
        int dir = numSearchs, esq = 0;
        int mid;
        while (esq <= dir) {
            mid = (esq + dir)/2;
            if (search.equals(name[mid])) {
                return "SIM";
            } else if (search.compareTo(name[mid]) > 0) {
                esq = mid + 1;
            }else{
                dir = mid - 1;
            }
    ...
    public static void orderNames(String names[], int num) {
        String menor;
        String swap;
        int po;
