...
int main(void){
    ...
    char comandAuxiliar[MAX_LINE_SIZE]; //Variável que armazena o comando
    char positionAuxiliar[MAX_LINE_SIZE]; //Variável que armazena a posição
    char arqAux[MAX_LINE_SIZE]; 
    ...
    for(int i = 0; i <breakpoint; i++){
        ...
        substring(comandAuxiliar, line, 0, 2);
        
        //Caso o comando dado seja INSERIR NO INICIO
        if(comandAuxiliar[0] == 'I' && comandAuxiliar[1] == 'I'){
            substring(arqAux, line, 3, strlen(line)-1);
            strcpy(caminhoArquivo, PREFIXO);
            strcat(caminhoArquivo, arqAux);
        ...
        else if(comandAuxiliar[0] == 'I' && comandAuxiliar[1] == '*'){
            substring(positionAuxiliar, line, 3, 5);
            position = atoi(positionAuxiliar);
            substring(arqAux, line, 6, strlen(line));
            strcpy(caminhoArquivo, PREFIXO);
            strcat(caminhoArquivo, arqAux);
            remove_line_break(caminhoArquivo);
            char *html = ler_html(caminhoArquivo);
        ...
        else if(comandAuxiliar[0] == 'I' && comandAuxiliar[1] == 'F'){
            substring(arqAux, line, 3, strlen(line));
            strcpy(caminhoArquivo, PREFIXO);
            strcat(caminhoArquivo, arqAux);
            remove_line_break(caminhoArquivo);
            char *html = ler_html(caminhoArquivo);
        ...
        else if(comandAuxiliar[0] == 'R' && comandAuxiliar[1] == 'I'){
            printf("(R) %s\n", removerInicio().nome);
        }

        //Caso o comando dado seja RETIRAR EM QUALQUER POSIÇÃO
        else if(comandAuxiliar[0] == 'R' && comandAuxiliar[1] == '*'){
            substring(positionAuxiliar, line, 3, 5);
            position = atoi(positionAuxiliar);
        ...
        else if(comandAuxiliar[0] == 'R' && comandAuxiliar[1] == 'F'){
            printf("(R) %s\n", removerFim().nome);
        }
